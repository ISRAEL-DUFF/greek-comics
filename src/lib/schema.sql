...
CREATE TABLE IF NOT EXISTS public.notes (
    ...
    folder_path text,
    notebook_book_id bigint REFERENCES public.notebook_books(id),
    page_order integer
);

-- Create Notebook Books Table
CREATE TABLE IF NOT EXISTS public.notebook_books (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    title text NOT NULL,
    is_pinned boolean DEFAULT false
);
ALTER TABLE public.notebook_books ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow public read access" ON public.notebook_books;
CREATE POLICY "Allow public read access" ON public.notebook_books FOR SELECT USING (true);
DROP POLICY IF EXISTS "Allow public write access" ON public.notebook_books;
CREATE POLICY "Allow public write access" ON public.notebook_books FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "Allow public update access" ON public.notebook_books;
CREATE POLICY "Allow public update access" ON public.notebook_books FOR UPDATE USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Allow public delete access" ON public.notebook_books;
CREATE POLICY "Allow public delete access" ON public.notebook_books FOR DELETE USING (true);
...
ALTER TABLE public.notes DROP COLUMN IF EXISTS book_id;
ALTER TABLE public.notes ADD COLUMN IF NOT EXISTS notebook_book_id bigint REFERENCES public.notebook_books(id);
ALTER TABLE public.notes ADD COLUMN IF NOT EXISTS page_order integer;
