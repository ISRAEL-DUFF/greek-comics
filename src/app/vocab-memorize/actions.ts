...
export type BookData = GenerateVocabMemorizeBookOutput & {
...
export async function generateBookAction(
  ...
  const validatedFields = VocabMemorizeFormSchema.safeParse({
  ...
  try {
    const bookPromise = generateVocabMemorizeBook(validatedFields.data);
    const coverPromise = generateBookCover("Vocabulary Story", validatedFields.data.vocabList);
    ...
    return { 
      data: {
        ...
        vocabList: validatedFields.data.vocabList,
