...
import { AlertCircle, BookOpen, Download, FileJson, ImagePlus, Image as ImageIcon, Languages, Maximize, WholeWord, Library, Save, Loader2 } from 'lucide-react';
...
interface VocabMemorizeDisplayProps {
...
export function VocabMemorizeDisplay({ 
    bookResult, 
    isLoading, 
    onEnterFullscreen,
    showImages,
  ...
  const handleExportJson = () => {
    ...
    const topicSlug = 'vocab-memorize-story';
    ...
    return (
      <Card className="flex flex-col items-center justify-center p-8 text-center border-dashed min-h-[60vh]">
        ...
        <h3 className="mt-4 text-xl font-semibold font-headline">
          Your Vocabulary Story Awaits
        </h3>
        <p className="mt-2 text-muted-foreground">
          Use the form to generate a new story.
        </p>
      </Card>
    );
  }

  const { title, author, pages, coverIllustrationUri, level, vocabList, grammarScope } = currentBook;
  ...
  const renderParagraphText = (p: { text: string, translation: string }, isClickable: boolean) => {
    ...
    if(isClickable) {
      ...
      return words.map((word, index) => (
        <WordClickPopover key={index} word={word} onAddWord={() => onAddWordToPanel(word.replace(/[.,·;]/g, ''))} />
      ));
    } else {
      return fullText;
    }
          ...
          <div className="no-print flex justify-end gap-4 flex-wrap items-center">
                <div className="flex items-center space-x-2 mr-auto flex-wrap gap-y-2">
                    <div className="flex items-center space-x-2">
                     ...
                     <div className="flex items-center space-x-2">
                        <Switch id="show-word-click" checked={isWordClick} onCheckedChange={setIsWordClick} />
                        <Label htmlFor="show-word-click" className="flex items-center gap-2 text-sm"><WholeWord className="h-4 w-4" />Word Analysis</Label>
                    </div>
                </div>
              ...
              {!!process.env.NEXT_PUBLIC_SUPABASE_URL && !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY && (
                <Button
                  onClick={async () => {
                    ...
                    setIsSaving(false);
                    if (res.success) {
                      toast({ title: 'Story Saved!', description: 'Your story has been saved to the cloud.' });
                    } else {
                      ...
                      <Card className="p-4 md:p-6 border-primary border-2">
                          <div className="flex flex-col items-center text-center gap-4">
                              ...
                              <div className="mt-2 text-sm text-muted-foreground max-w-xl">
                                <span className='font-semibold'>Vocabulary:</span> {vocabList}
                              </div>
                          </div>
                      </Card>
                  </CarouselItem>
